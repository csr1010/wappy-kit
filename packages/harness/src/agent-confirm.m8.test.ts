...
describe("createAgent — confirm-before-write flow (T8.5)", () => {
  test("without deps.confirmFlow configured, a 'confirm' selectionId is routed normally (no interception)", async () => {
  ...
  test("a 'confirm' with nothing pending gets an honest 'nothing to confirm' reply, router never runs", async () => {
    const channel = fakeChannel("whatsapp");
    const memory = fakeMemory();
    const router = fakeRouter([GREETING]);
    let routed = false;
  ...
  test("confirm executes the pending tool exactly once, with the persisted args, and replies 'Done'", async () => {
    ...
    const memory = fakeMemory();
    const confirmFlow = createConfirmFlow({ client: createClient({ url: ":memory:" }), clock: fakeClock() });
    await confirmFlow.request({ contactId: "c1", toolName: "cancelOrder", args: { id: "1001" }, summary: "cancelOrder with 1001" });
  ...
  test("a duplicate 'confirm' delivery (retry) after the first is fully handled does NOT re-execute the tool", async () => {
    ...
    await agent.handle(msg({ id: "m1", selectionId: CONFIRM_SELECTION_ID, text: "Confirm" }));
    const second = await agent.handle(msg({ id: "m2", selectionId: CONFIRM_SELECTION_ID, text: "Confirm" }));
  ...
  test("cancel discards the pending confirmation without ever executing the tool", async () => {
    ...
    const result = await agent.handle(msg({ selectionId: CANCEL_SELECTION_ID, text: "Cancel" }));
  ...
  test("a pending confirmation for a tool no longer in deps.tools fails honestly instead of crashing", async () => {
    ...
    const memory = fakeMemory();
    const confirmFlow = createConfirmFlow({ client: createClient({ url: ":memory:" }), clock: fakeClock() });
    await confirmFlow.request({ contactId: "c1", toolName: "goneTool", args: {}, summary: "goneTool" });
