...
const readPartVersionsMock = vi.fn(() => ({ core: "0.1.0", harness: "0.1.0", whatsapp: "0.1.0", toolsOpenapi: "0.1.0", createWappy: "0.1.0" }));
...
describe("bin.ts's main() — wires real argv/cwd/versions/interactive into runCli", () => {
  test("passes process.argv (minus the node/script args), process.cwd(), and the real version lookup through to runCli", async () => {
    ...
    expect(call.versions).toEqual({ core: "0.1.0", harness: "0.1.0", whatsapp: "0.1.0", toolsOpenapi: "0.1.0", createWappy: "0.1.0" });
